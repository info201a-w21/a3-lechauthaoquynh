...
#What is the average value of black jail population across all the counties 
# most recently? `recent_average_black_jail_pop`
...
black_jail_pop_over_time_in_top_5 <- ggplot(data = top_5_chart) +
  ...
  geom_smooth(mapping = aes(x = year, y = black_jail_pop, color = county_name),
              se = FALSE) +
  labs(x = "Year", y = "Black Jail Population", title = "Top 5 Counties Has Highest
...
two_variables_data <- data.frame(
  incarceration_trends$black_jail_pop, incarceration_trends$white_jail_pop)
